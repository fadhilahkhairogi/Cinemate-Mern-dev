...
function Order() {
  ...
  const handlePickSeat = () => {
    ...
    function getOrderTime() {
      ...
      if (day >= 1 && day <= 4) return 35000
      if (day === 5) return 40000
      return 50000
    }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                    ...
                    <div className="md:col-span-5 bg-gray-100 text-black rounded-[15px] p-4 sm:p-5">
                      ...
                      <div className="flex flex-wrap gap-3 mb-3 font-semibold justify-between text-sm sm:text-base">
                        ...
                        <div className="flex items-center">
                          <span className="w-4 h-4 sm:w-5 sm:h-5 bg-[#697079] rounded mr-2" /> Taken
                        </div>
                        <div className="flex items-center">
                        ...
                        {['A', 'B', 'C', 'D', 'E', 'F', 'G'].map(row => (
                          <div key={row} className="flex items-center mb-1 text-sm">
                            ...
                            {[...Array(10)].map((_, i) => {
                              ...
                              let seat = `${row}${seatIndex}` //is in the list seat Taken
                              if (listTakenSeat && listTakenSeat.includes(seat)) {
                                return (
                                  <React.Fragment key={`${row}${seatIndex}`}>
                                    {/* //     //       <div
                              //     //         //   type="checkbox"
                              //     //         //   value={`${row}${seatIndex}`}
                              //     //         className="seat-checkbox appearance-none w-5 h-5 sm:w-6 sm:h-6 m-1 bg-[#000000] border border-gray-300 rounded cursor-pointer checked:bg-linear-to-br checked:from-[#00A3FB] checked:to-[#000000]"
                              //     //       /> */}
                                    <div
                                      // key={`${row}${seatIndex}`}
                                      className="w-5 h-5 sm:w-6 sm:h-6 m-1 bg-[#697079] border border-gray-400 rounded cursor-not-allowed"
                                    />
                                    {/* // //{' '}
                                    <s
                                      pan
                                      className="w-4 h-4 sm:w-5 sm:h-5 bg-[#697079] rounded mr-2"
                                    /> */}
                                    {isAfterGap && <div className="w-6 m-1" />}
                                  </React.Fragment>
                                )
